...
spl_autoload_register(function ($class) {
    ...
    $class_attr = explode('\\', $class);
    $namespace = '';

    if (count($class_attr) > 1) {
        $namespace = $class_attr[0];
        $classname = $class_attr[1];

    } else {
        $classname = $class_attr[0];
    }
...
class VendAPI
{
     ...
     */
    private function _request($path, $data = null, $update = false,$urlencode=false,$delete=false, $depage = null)
    {
        ...
        if ($result === null) {
            die('<h3 class="notice-error">Please Check Your Credentials & Internet Connection</h3>');
        }
    ...
    {
        $result = $this->_request('api/2.0/customers' . $path);
	...
	public function manageWebhook($data = null){

    	if($data){
		    $result = $this->requestr->post('/api/webhooks',$data);
	    }else{
		    $result = $this->_request('/api/webhooks');
	    }
    ...
    public function get_all_products($options = array())
    {
        ...
        $result = $this->_request('/api/2.0/products');
        if (!isset($result->data) || !is_array($result->data)) {
     ...
     * @return object
     */
    public function saveCustomer($cust, $update=false, $path = null)
    {
    	if($update){
            $result = $this->_request('/api/2.0/customers/'.$path, $cust->saveArray(),true);
        }else{
            $result = $this->_request('/api/2.0/customers', $cust->saveArray());
        }

        return new VendCustomer($result->data, $this);
    }
